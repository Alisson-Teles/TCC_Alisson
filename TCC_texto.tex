...
Nas subceções abaixo são descritos três trabalhos, ressaltando suas contribuições e limitações quanto ao escopo deste projeto.
...
A metodologia será composta por etapas sequenciais, confome apresentado na figura \hyperref[fig:minhafigura7]{Figura~\ref*{fig:minhafigura7}}. Nossa metodologia começa pela preparação dos dados, em seguida  a utilização de um \textit{parser} específico, depois o treinamento de modelo e por fim a avaliação do modelo treinado. Primeiramente, selecionam-se os conjuntos de dados de programas em C que servirão de entrada, conforme a primeira parte do fluoxograma. Em seguida, implementa-se um conversor para traduzir esses programas para a ROBL. Após a conversão, os dados são organizados em pares de entrada (\textit{prompt}) e saída (resposta/\textit{label}) para treinamento do modelo. O modelo de linguagem é então treinado (ajuste fino) utilizando-se o \textit{LLM Compiler}, adaptando-o à tarefa de otimização de código específico da plataforma STM32. Por fim, realiza-se a validação do modelo treinado, comparando o código gerado com otimizações convencionais.

Na fase de validação e experimentos, o desempenho do modelo treinado será avaliado com base em sua capacidade de reduzir o tamanho do código e manter a compilabilidade dos programas. Serão utilizados códigos do conjunto de validação, não vistos durante o treinamento, e para cada entrada será gerado um código otimizado pelo modelo. Esse resultado será comparado com a saída obtida ao aplicar a flag padrão -Oz do compilador, utilizando como métrica principal a contagem de instruções geradas na IR. Para que o código seja considerado validado, ele deverá não apenas reduzir o tamanho em relação ao -Oz, mas também ser compilável. Os testes serão conduzidos de forma automatizada, com medições para quantificar os ganhos e perdas, fornecendo uma base objetiva para verificar a eficácia do modelo. O objetivo final é verificar se o modelo treinado é capaz de replicar ou superar as otimizações tradicionais em um contexto de arquitetura restrita, como a de microcontroladores.
...
Para compor a base de entrada do pipeline experimental, serão utilizados dois conjuntos de programas escritos em C, visando abranger tanto códigos sintéticos aleatórios quanto códigos oriundos de aplicações reais. O primeiro conjunto é gerado pelo Csmith \cite{yang2011finding}, uma ferramenta que produz programas em C aleatórios. O Csmith é comumente empregado para \textit{stress testing} de compiladores, garantindo uma ampla diversidade de construções nos programas de entrada. O segundo conjunto de programas é o AnghaBench \cite{da2021anghabench}, uma suíte que contém aproximadamente um milhão de funções em C compiláveis extraídas de repositórios públicos do GitHub. Diferentemente do código sintético do Csmith, o AnghaBench provê códigos representativos de padrões e estruturas encontrados em softwares do mundo real.

Essa combinação de \textit{datasets} complementares visa maximizar a diversidade do conjunto de treino - códigos aleatórios exercitam aspectos incomuns da linguagem, enquanto códigos reais fornecem exemplos fielmente extraídos de aplicações práticas.

Como o Robcmp não pode compilar código C, os conjuntos gerados serão convertidos de C para ROBL usando um conversor (c2rob) descrito na próxima seção.

\section{Conversor de código C para RobCmp: c2rob}

Para viabilizar a pesquisa, desenvolveu-se um conversor capaz de traduzir programas escritos em C para a sintaxe da ROBL, chamado \texttt{c2rob}.\footnote{Disponível em https://github.com/thborges/c2rob} A implementação foi realizada com auxílio das ferramentas Flex (versão 2.6.4) e Bison, escolhidas por sua capacidade de construir analisadores léxicos e sintáticos customizados.

O \texttt{c2rob} lê o código fonte em C, proveniente dos \textit{datasets} descritos, e o transforma em um código equivalente na sintaxe da ROBL, preservando a lógica e o comportamento do programa original.

Como nem todos os recursos da linguagem C estão presentes na ROBL, uma etapa adicional será empregada para selecionar códigos possíveis de conversão. A própria falha de tradução, apresentada pelo \texttt{c2rob} como erro de sintaxe, será um indicador de incompatibilidade. O desenvolvimento do \texttt{c2rob} é recente e uma fase adicional de ajustes finos e testes será realizada no início da execução do projeto.

Buscamos atingir o mínimo de 100.000 códigos, sendo 80\% deste valor para o treinamento, 10\% para monitorar o desempenho e evitar \textit{overfitting} durante o treinamento e, por fim, os 10\% restantes comporão o conjunto de inferência, que será usado para avaliar a capacidade de generalização e a eficácia do modelo após o treinamento.

\section{Formação dos Dados de Treinamento}

Com os códigos convertidos, procede-se à organização dos dados em pares de \textit{prompt} e resposta para alimentar o treinamento supervisionado do modelo de linguagem. Em cada par, o \textit{prompt} consiste de um comando inicial textual em inglês, seguido do código em LLVM IR para a plataforma alvo (ROBL compilado para LLVM IR, sem otimização), enquanto o \textit{label} corresponde à melhor versão compilada do mesmo programa em \textit{assembly} e os respectivos passes de otimização empregados, seguindo o padrão adotado para o treinamento do modelo original \cite{cummins_llm_2025}.

Para identificar a melhor versão compilada, será empregado um processo de \textit{autotuning}, inspirado na metodologia descrita por \citeonline{faustino2021new}. A metodologia busca encontrar sequências de otimização menores e mais eficazes para reduzir o tamanho do código objeto, usando uma ferramenta chamada \textit{YaCoS} \footnote{Repositório do YaCoS: \url{https://github.com/ComputerSystemsLaboratory/YaCoS}}, que gera milhares de sequências de otimização. Cada sequência é representada como um vetor de passes do LLVM, evoluído em populações de 100 indivíduos ao longo de 50 gerações. A melhor sequência para cada função é então reduzida com o algoritmo de \citeonline{purini2013finding}, removendo passes desnecessários e duplicados. O resultado é uma matriz de otimização (programas × sequências), usada para selecionar cinco sequências com base em diferentes métricas: frequência de melhor resultado, proximidade do ótimo, soma total, média geométrica e pior caso.

O esquema de treinamento e inferência é apresentado na \hyperref[fig:minhafigura6]{Figura~\ref{fig:minhafigura6}}. O \textit{prompt}  contém um texto em inglês perguntando qual os passes necessários para se reduzir a contagem de instruções existentes no código IR e o código não otimizado. A resposta (\textit{answer}) contém os passes de otimização necessários para reduzir a contagem de instruções de 15 para 4, por exemplo. Durante a inferência, extraímos a lista de passes de otimização da resposta, que é, então, usada no compilador para produzir o código otimizado que será usado em outras validações descritas na \autoref{sec:validacao}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{imagens/fig6.png}
    \caption{Esquema de Treinamento e Inferência do LLM Compiler para STM32. Figura adaptada de \citeonline{cummins_large_2023}}
...
\section{Validação do Modelo Refinado} \label{sec:validacao}

Para a validação do modelo treinado, serão reservados 10\% do total de dados processados, resultando em um conjunto de validação com, no mínimo, 10.000 amostras nunca vistas durante o treinamento. O objetivo é verificar a capacidade de generalização do modelo treinado e sua eficácia em realizar otimizações sobre códigos voltados à arquitetura STM32.

Durante os testes, o modelo receberá como entrada o código não otimizado em LLVM IR (compilado a partir da linguagem Robcmp), e deverá gerar como saída uma sequência de passes de otimização. Esta sequência será então aplicada ao código original, resultando em uma nova versão que será comparada sob diferentes métricas.

A principal métrica de desempenho será a redução percentual no tamanho do código final em comparação com o que seria gerado pelo compilador usando a \textit{flag} padrão -Oz, replicando a abordagem descrita por \citeonline{cummins_llm_2025}. Também será usada a métrica \textbf{MAPE} (Erro Percentual Absoluto Médio), que avalia o quão precisa foi a estimativa do modelo sobre o tamanho do código gerado após otimização. Isso ajuda a entender se o modelo está prevendo bem o impacto das mudanças que sugere.
...
Por fim, para verificar a precisão das otimizações aprendidas, será considerada a métrica de correspondência exata (\textit{Exact Match}) com o código gerado pelo \textit{autotuning}, isto é, quantos dos códigos otimizados pelo modelo coincidem exatamente com os gerados pelo processo de busca automática de passes descrito por \citeonline{faustino2021new}.

Todo o processo de validação será automatizado por meio de \textit{scripts} que executam os testes em lote, avaliam as instruções geradas e contabilizam as diferenças de desempenho entre os métodos. Os resultados serão analisados de forma quantitativa, com médias e percentuais de redução, possibilitando uma avaliação objetiva da eficácia do modelo em otimizar código para a arquitetura STM32.


\section{Especificações de Hardware e Software}

Quanto aos recursos utilizados, o \texttt{c2rob} será aprimorado e testado com auxílio do Visual Studio Code versão 1.101.0, no Ubuntu 24.04 LTS, executado em um \textit{desktop} Lenovo ThinkCentre M93p, com processador Intel® Core™ i5-4570 (4 núcleos), 32GB de RAM e gráficos integrados Intel® HD \textit{Graphics} 4600. Essa infraestrutura local será utilizada principalmente nas etapas de geração, conversão e manipulação dos dados, enquanto o treinamento do modelo propriamente dito ocorrerá em ambiente de alto desempenho.
...
Geração dos datasets com Csmith e AnghaBench & Julho (2ª quinzena) & 2025 \\
 Testes do parser com códigos gerados& Agosto (1ª quinzena) & 2025 \\
 Geração dos pares prompt/label com autotuner& Agosto (2ª quinzena) & 2025 \\
 Organização dos dados para fine-tuning& Setembro (1ª quinzena) & 2025 \\
 Execução do fine-tuning do modelo no Hugging Face& Setembro (2ª quinzena) & 2025 \\
 Inferência e validação dos códigos gerados (comparação com -Oz)& Outubro (1ª quinzena) & 2025 \\
 Análise quantitativa e documentação (escrita) dos resultados& Outubro (2ª quinzena) & 2025 \\
