...
Para realizar a criação dos dados de treinamento, é necessário que os pares de \textit{prompt-label}, possuam em seu label uma versão compilada que se aproxime em eficiência da \textit{flag} -Oz. Para isso será empregado um processo de \textit{autotuning}, aproveitando uma lista de passes, que é um dos resultados do trabalho de \citeonline{faustino2021new}, conhecida como \textit{Optimization Cache}\footnote{Disponível em: \url{https://zenodo.org/records/4416117}} que funcionam como um caso geral para os códigos, assim como a \textit{flag} -Oz. Para realizar o procedimento de aplicar essas sequências em nossos códigos ROBL, criamos um \textit{pipeline} de execução\footnote{Disponível em: \url{https://github.com/Alisson-Teles/Pipeline-training}}. 
